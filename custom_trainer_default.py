...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        """
        output_folder = "outputs_single"
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)

        output_file_name = f"{output_folder}/output_{cur_id[0]}.txt"
        ...
        # save labels
        labels_file_name = f"{output_folder}/labels_{cur_id[0]}.txt"
    ...
    def compute_loss_ok(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """
        How the loss is computed by Trainer. By default, all models return the loss in the first element.
        ...
        # insepct the model function
        # actually print it out
        import inspect
        #rank0_print(inspect.getsource(model.module.forward))
        #print(model.module.loss_function)
        #rank0_print(inspect.getsource(model.module.loss_function))
        #rank0_print(inspect.getsource(model.module.loss_function.__globals__['fixed_cross_entropy']))
        #print(inspect.getsource(fixed_cross_entropy))

        parameter_loss = F.cross_entropy(outputs.logits.view(-1, outputs.logits.size(-1)), labels.view(-1), ignore_index=LLAMA_IGNORE_INDEX)
        cur_id = inputs["id"]
        # if rank is 0
        #if dist.get_rank() == 0:
        print("OUTPUT_PRINT gpu: ", dist.get_rank(), "id: ", cur_id, "outputs: ", outputs)
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    if do_sync_step: # not hitting here
                        ...
                        #print(f"gpu: {dist.get_rank()}, {i}: 7")

                        # sync tr_loss here: 
                        """
                        if dist.is_initialized():
                            tr_loss_detached = tr_loss.detach()
                            dist.all_reduce(tr_loss_detached, op=dist.ReduceOp.SUM)
                            tr_loss_detached /= dist.get_world_size()
                        else:
                            tr_loss_detached = tr_loss.detach()
                        """
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if losses is not None:
                if not torch.isfinite(losses).all():
                    print(f"[Eval] Skipping batch with non-finite loss: {losses}")
